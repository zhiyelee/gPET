       ...
       /*    for (;;)
            {
                ...
                if (nsstktemp> NSSTACKSHARED) break;
							...
							sftemp[ind] = xtemp.x; // to mm
							sftemp[ind+1] = xtemp.y;
							sftemp[ind+2] = xtemp.z;
							sftemp[ind+3] = xtemp.w;//s;
				...
				/* if(id ==nactive-1 )
	            printf("initial: xtemp.x = %f, xtemp.y = %f, xtemp.z = %f,xtemp.w = %f, vxtemp.x = %f, vxtemp.y = %f, vxtemp.z = %f, vxtemp.w =%f\n", xtemp.x, xtemp.y, xtemp.z,xtemp.w, vxtemp.x, vxtemp.y, vxtemp.z, vxtemp.w); 	  */
//              __syncthreads();
