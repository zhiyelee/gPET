################################################################################
# This is a modified version of the "common.mk" file from the NVIDIA CUDA SDK.
################################################################################
#
# Original license:
################################################################################
...
EXECUTABLE	:= gbrachy_test

# Note: must use right sm number here
#CUFILES_sm_13 	:= main.cu
CUFILES_sm_30 	:= main.cu
...
# Link with CUTIL and SHRUTIL
#USE_CUTIL_LIB   := 1
#USE_SHRUTIL_LIB := 1

# Use some CUDA libraries
#USECUFFT    := 1
USECUBLAS   := 1
USECURAND   := 1
#USECUSPARSE := 1
...
#LIB+= /usr/local/apps/argtable2-13/lib/libargtable2.a
LIB+= -largtable2
...
# Comment this line to put the executable in a "debug" or "release" subdirectory
BINSUBDIR  := .
...
# SDK-related includes
INCLUDES  += -I$(CUDA_SDK_PATH)/C/common/inc -I$(CUDA_SDK_PATH)/shared/inc/
...
# Libs
ifneq ($(DARWIN),)
    LIB       += -L$(CUDA_INSTALL_PATH)/lib -L$(LIBDIR) $(NVCUVIDLIB) 
else
  ifeq "$(strip $(HP_64))" ""
    ifeq ($(x86_64),1)
...
# SDK libs
LIB += -L$(CUDA_SDK_PATH)/C/lib/

# If dynamically linking to CUDA and CUDART, we exclude the libraries from the LIB
ifeq ($(USECUDADYNLIB),1)
     LIB += ${OPENGLLIB} -ldl -rdynamic 
else
...
ifneq ($(STATIC_LIB),)
	...
	# CUTIL and SHRUTIL libraries
	ifeq ($(USE_CUTIL_LIB),1)
		#LIB += -lcutil_$(LIB_ARCH)$(LIBSUFFIX) 
		LIB += -lcutil
	endif
	ifeq ($(USE_SHRUTIL_LIB),1)
...
#    .cu file.
#
# The intended use for this is to allow Makefile to compile
...
#	$(VERBOSE)rm -f $(ROOTBINDIR)/$(BINSUBDIR)/*.txt
